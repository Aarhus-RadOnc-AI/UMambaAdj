...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
        ...
        if self.verbose:
            print(f'input run case npy data shape: {data.shape}')
        ...
        tanspose_value = [0, *[i + 1 for i in plans_manager.transpose_forward]]
        if self.verbose:
            print(f'transpose_forward value: {tanspose_value}')
        if seg is not None:
            seg = seg.transpose([0, *[i + 1 for i in plans_manager.transpose_forward]])
        original_spacing = [properties['spacing'][i] for i in plans_manager.transpose_forward]
        if self.verbose:
            print(f'data shape after  transpose_forward: {data.shape}')
        ...
        print(f'!!!!! warning cropping is removed for hnts challenge from "data, seg, bbox = crop_to_nonzero(data, seg)" !!!!!!!')
        print(f'!!!!! warning cropping is removed for hnts challenge, to "bbox = [[0, s] for s in data.shape[1:]]" !!!!!!!')
        nonzero_mask = create_nonzero_mask(data)
        bbox = [[0, s] for s in data.shape[1:]]
        nonzero_mask = nonzero_mask[None]
        if seg is not None:
            seg[(seg == 0) & (~nonzero_mask)] = -1
        else:
            seg = np.where(nonzero_mask, np.int8(0), np.int8(-1))
        # end of the code change.
        print("data shape by iterate after the crop process:", data.shape)
        print("seg shape by iterate after the crop process:", seg.shape)
        ...
        if self.verbose:
            print(f'bbox_used_for_cropping: {bbox}')
            print(f'shape_before_cropping: {shape_before_cropping}')
            print(f'shape_after_cropping_and_before_resampling: {data.shape}')
