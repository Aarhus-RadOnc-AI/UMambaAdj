...
def _maybe_copy_splits_file(splits_file: str, target_fname: str):
    print("target_fname->>>>>>>>>>>>", target_fname)
    if not isfile(target_fname):
        shutil.copy(splits_file, target_fname)
    else:
        print("split already exists, do not copy, but check that the splits match.")
        ...
        for i in range(len(splits_source)):
            ...
            print("train_target->", len(train_source))
            print("train_source->",len(train_target))
